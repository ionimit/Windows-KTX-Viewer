...
- macOS-style title bar overlay on exports (filename as window chrome)
...
├── ktxview.py              # SSH terminal previewer (not built as exe)
...
## Title Bar Overlay Format

The `ktx_cli.exe` adds a macOS-style title bar when exporting:
- **Height:** 38 px
- **Background:** Dark grey (`#323232`)
- **Traffic lights:** Red, yellow, green dots (top-left)
- **Filename:** Centered, 23 px bold text
- **Result:** Exports look like screenshots from a native image viewer

Disable with `--no-title` flag.
